...
fn auto_grant_access_if_needed(ip: &str, server: &str) {
    ...
    let exists = data.iter().any(|a|
        a.ip_address == ip && a.server_id == server && a.valid_until.map(|v| v > Utc::now()).unwrap_or(false));
    ...
    if !exists {
        ...
        fs::write(&path, serde_json::to_string_pretty(&data).unwrap()).unwrap();
        println!("Automatically granted temporary access to IP {} for server {}", ip, server);
    }
}

fn launch_monitored_shell(ip: &str, server: &str) {
    println!("Launching monitored shell for IP '{}' on server '{}'...", ip, server);
    ...
    loop {
        ...
        let session_file = format!("user_logs/{}_{}.jsonl", ip.replace(".", "_"), entry_time.format("%Y%m%dT%H%M%S"));
